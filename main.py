...
# =================== MULTI-TARGET SPAM PROCESS ENGINE (SPEED BOOST) ===================
async def run_unlimited_spam(username: str, accounts, target_uid: str, region: str, badges_str: str, fast_mode: bool):
    ...
    while target_data["is_running"] and not target_data["stop_requested"]:
        ...
        # 🔥 স্পিড বুস্ট: ব্যাচ প্রসেসিং (10 অ্যাকাউন্ট একসাথে)
        batch_size = 10
        for i in range(0, len(accounts), batch_size):
            batch = accounts[i:i+batch_size]
            
            tasks = []
            for bot_uid, password in batch:
                if not target_data["is_running"] or target_data["stop_requested"]:
                    break
                tasks.append(process_single_spam(username, target_data, bot_uid, password, target_uid, region, badges_str, fast_mode))
...
async def process_single_spam(username, target_data, bot_uid, password, target_uid, region, badges_str, fast_mode):
    """একটি স্প্যাম প্রসেস - আলাদা ফাংশন (স্পিড বুস্ট)"""
    try:
        ...
        try:
            ...
            for badge_name in badges_to_send:
                ...
                delay = 0.2 if fast_mode else 0.5
        ...
        # স্পিডের জন্য লগ না করে চালিয়ে যান
        pass
